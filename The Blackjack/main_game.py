...
def deal_card(list):
    '''Returns a card randomly from a list''' 
...
def compare_points(user_cards, computer_cards):
    '''Compares the points of the user and the computer and returns the result'''
...
def play_game():
    ...
    second_card = input("Do you want another card? Type 'y' or 'n': ").lower()
    if second_card == 'y':
        user_cards.append(deal_card(cards))
        ...
        print(f"Your cards: {user_cards}, current score: {calculate_score(user_cards)}")
        print(compare_points(user_cards, computer_cards))
    else:
        while calculate_score(computer_cards) < 17:
            computer_cards.append(deal_card(cards))
        print(f"Computer's final hand: {computer_cards}, final score: {calculate_score(computer_cards)}")
